...
import { SimpleLineIcons, MaterialIcons, MaterialCommunityIcons, AntDesign, FontAwesome } from '@expo/vector-icons';
...
architectureNames = [ 'door_1.png', 'door_2.png', 'stairs_1.png', 'window_1.png' ];
...
export default class ImageEditor extends Component {
		...
		if (this.state.selectColor) {
			return (
				...
				>
					...
					/>
					<TouchableOpacity style={{ padding: 5, alignItems: 'center' }} onPress={this.changeColor}>
						<SimpleLineIcons style={{ margin: 5 }} name="close" size={30} color="white" />
					</TouchableOpacity>
				</View>
			);
		} else {
	...
	_renderElementsPanel = () => {
		...
		furnitureNames.forEach((image, i) => {
			let thisImage = (
				<TouchableOpacity
					style={styles.icon}
					onPress={() => this.addToScene(api + '/uploads/elements/furniture/' + image)}
		...
		architectureNames.forEach((image, i) => {
			let thisImage = (
				<TouchableOpacity
					style={styles.icon}
					onPress={() => this.addToScene(api + '/uploads/elements/architecture/' + image)}
		...
		if (this.state.showElements) {
			return (
				...
				>
					<Text style={{ color: 'white', alignSelf: 'center', fontSize: 15 }}>Basic furniture elements</Text>
					<ScrollView
						horizontal
	...
	activateDrawPolyTool = () => {
		if (this.state.showPolyDrawer === false && this.state.showFillButton === false) {
			alert(
		...
		return (
			<View style={styles.container}>
				...
				>
					...
					<TouchableHighlight style={[ styles.icon, styles.rotateIcon ]} onPress={this.showSketch}>
						<FontAwesome name="paint-brush" size={25} color="white" />
					</TouchableHighlight>
					<TouchableHighlight style={[ styles.icon, styles.rotateIcon ]} onPress={this.incrementWidth}>
					...
					<TouchableOpacity style={styles.icon} onPress={this.activateDrawPolyTool}>
						<MaterialCommunityIcons name="drawing" size={25} color="white" />
					</TouchableOpacity>
					<TouchableOpacity style={styles.icon} onPress={this.openElementsPanel}>
				...
				>
					<View style={styles.sketchContainer}>
						...
						{this.state.showPolyDrawer && <ElementsViewer updatePoly={this.updatePoly} />}

						<ImageBackground source={{ uri: this.props.selected[0] }} style={styles.background}>
							{this.state.sketch && (
								<ExpoPixi.Sketch
